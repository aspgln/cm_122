...
if __name__ == "__main__":
    ...
    chr_name = '{}'.format(genome_name)
    reads_fn_end = '{}_reads.txt'.format(chr_name)
    reads_fn = join(input_folder, reads_fn_end)
    ref_fn_end = '{}_ref.txt'.format(chr_name)
    ref_fn = join(input_folder, ref_fn_end)
    start = time.clock()
    input_fn = join(input_folder, '{}_reads.txt'.format(chr_name))
