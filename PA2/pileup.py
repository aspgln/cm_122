...
def align_to_donor(donor, read):
    ...
    for shift_amount in range(-3, 0) + range(1, 4):  # This can be improved
        ...
        mismatches = [1 if donor[i] != ' ' and shifted_read[i] == donor[i] else 0 for i in range(len(donor))]
        n_mismatches = sum(mismatches)

        score = n_mismatches - 3 * abs(shift_amount)
        if score > best_score:
...
def edit_distance_matrix(ref, donor):
    ...
    tag_del = output_matrix
    tag_ins = output_matrix
    # print len(ref), len(donor)
    # print output_matrix, tag_del, tag_ins
    ...
    for j in range(1, len(donor)):
        for i in range(1, len(ref)):  # Big opportunities for improvement right here.
            deletion = output_matrix[i - 1, j] + (1 if tag_del[i - 1, j] == 1 else 5)
            insertion = output_matrix[i, j - 1] + (1 if tag_ins[i, j - 1] == 1 else 5)
            identity = output_matrix[i - 1, j - 1] - 1 if ref[i] == donor[j] else np.inf
            substitution = output_matrix[i - 1, j - 1] + 3 if ref[i] != donor[j] else np.inf
            tmp = min(insertion, deletion, identity, substitution)
            output_matrix[i, j] = tmp
            if tmp == insertion:
                tag_ins[i, j] = 1
            elif tmp == deletion:
                tag_del[i, j] = 1
